...
> 一个 LSPosed / Xposed 模块，在 DLsiteSound（DLsite 音频 App）的**播放页**上挂一个与播放进度同步的**系统级悬浮字幕窗**；1.21.4 起还会把当前字幕行镜像到**系统状态栏**（在 SystemUI 进程内注入）。
...
- 🆕 **状态栏字幕（1.21.4 起）**：把当前字幕行**镜像到系统状态栏** —— 在 SystemUI 进程里注入一个常驻 `FrameLayout`，由 App 进程**跨进程广播**推送字幕行（所以关掉悬浮窗，状态栏字幕照样在）。默认**关**，由播放页左侧的`状态栏 开 / 状态栏 关`胶囊按钮单击切换。详见 [docs/statusbar-subtitle.md](docs/statusbar-subtitle.md)。
...
- 🆕 **播放页双胶囊按钮（2.0.1）**：播放控制条上方并排两个 MD3 胶囊按钮 —— 左`状态栏 开 / 状态栏 关`（**无字幕时整个消失**），右`悬浮窗 开 / 悬浮窗 关 / 无字幕`。按钮**恒定 75×32dp 不压缩**，底边强制落在**滑条顶边往上 25dp**，不会被可用空间挤到进度条上。
- 🆕 **转场抖动防护（2.0.1）**：锁定视图密度（`stableDensity`），ColorOS 打开播放页那几秒 density 在 476 / 560 / 620dpi 间跳变时，按钮不再斜着抖（异常读数打 `density spike ignored`）。
...
1. 本项目主要用于个人技术研究 / 学习目的；作者**不鼓励**（但并不禁止）将其用于商业用途，具体授权范围以 [LICENSE](LICENSE) 为准。
