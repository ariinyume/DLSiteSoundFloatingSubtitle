...
- 🆕 **播放页双胶囊按钮（2.0.1 起）**：播放控制条上方并排两个 MD3 胶囊按钮 —— 左`状态栏 开 / 状态栏 关`（**无字幕时整个消失**），右`悬浮窗 开 / 悬浮窗 关 / 无字幕`。
  ...
  - `悬浮窗 开`：悬浮窗已打开
  - `无字幕`：当前音轨没有可用字幕
  - **尺寸固定（2.1.2 起）**：单个胶囊宽 `85dp`（2.1.2 之前是 `75dp`）。宽高、圆角、文字、距滑条偏移只按**首次锁定的密度**计算 —— 不再随系统「显示大小」变化（实测 ColorOS 的宿主密度会在屏幕缩放档位间来回跳，旧版因此让胶囊高度在 `95 / 112 / 124px` 之间反复变）；文字用**固定 px** 并按胶囊可用宽度自动适配，保证**完全显示**，也不受系统「字体大小」影响。
  - **多语言（2.1.2 起）**：按**系统语言**取词（读的是系统配置，不受宿主 App 的「应用语言」影响），简体中文保持原样。
    | 系统语言 | 左 / 右按钮 | 按钮`无字幕` | 悬浮窗占位 |
    | --- | --- | --- | --- |
    | 简体中文 / 中文（简体） | `状态栏 开 / 关`、`悬浮窗 开 / 关` | `无字幕` | `无字幕` |
    | 繁體中文 / 中文（繁體）（中國香港 / 中國台灣 / 香港 / 台灣） | `狀態欄 開 / 關`、`懸浮窗 開 / 關` | `無字幕` | `無字幕` |
...
- 🆕 **状态栏字幕（2.0.1 起）**：把当前字幕行**镜像到系统状态栏** —— 在 SystemUI 进程里注入一个常驻 `FrameLayout`，由 App 进程**跨进程广播**推送字幕行（所以关掉悬浮窗，状态栏字幕照样在）。默认**关**，由播放页左侧的`状态栏 开 / 状态栏 关`胶囊按钮单击切换。详见 [docs/statusbar-subtitle.md](docs/statusbar-subtitle.md)。
  ...
  - **暂停宽限 0.4s**（`HIDE_GRACE_MS = 400ms`）：暂停后先短暂保留（宽限是为了让切页 / 换轨造成的瞬时暂停不闪出时钟），再隐藏并还原时钟 / 通知图标；恢复播放自动回来。
- **可单独使用悬浮窗字幕**：未授权 `com.android.systemui` 作用域时不显示状态栏字幕开关，用跨进程握手确认 SystemUI 是否已被注入（App 每 3s 发 PING、SystemUI 回 PONG，8s 内无回应即视为未授权），未授权时左侧`状态栏 开 / 状态栏 关`按钮不显示。
...
- **换轨智能处理**：切到无字幕音轨时先挂起、悬浮窗显示`无字幕`占位，等新音轨的字幕 JSON —— 无缓存 cues 时等 **3s**，已有缓存 cues 时放宽到 **10s**；**5s（2.1.2 起，此前 4s）**内还没等到 JSON 就**提前收窗**（只关窗、不下结论，避免长时间挂着占位），JSON 到了会**自动把窗口开回来**；直到裁决窗到点仍无 JSON，才清空 cues 并判「无字幕」。详见 [docs/track-change.md](docs/track-change.md)。
...
- 设备： 一加 15 ColorOS 16.0.9.400(CN01) 一加 12 ColorOS 16.0.10.501(CN01)
...
3. **授予悬浮窗权限**：
   - 第一步：设置 → 应用 → DLsiteSound → 权限管理 → 特殊应用权限 → `悬浮窗`。
   - 第二步：设置 → 应用 → DLsiteSound Floating Subtitle → 权限管理 → 特殊应用权限 → `悬浮窗`。
   - 如有需要，可允许`后台弹出界面`
4. **打开有字幕的播放页**：播放控制条上方出现两个胶囊按钮。
   - 左`状态栏 开 / 状态栏 关`：单击开关**状态栏字幕**（默认关）；无字幕时该按钮整个消失；未授权 `com.android.systemui` 作用域时该按钮不出现。
...
> GPL-3.0 是**传染性（copyleft）**许可：你可以自由使用、修改、再分发本项目，但**衍生作品必须同样以 GPL-3.0 开源**。若把本项目并入闭源程序，该程序的相关部分也需要按 GPL-3.0 开放源码。
